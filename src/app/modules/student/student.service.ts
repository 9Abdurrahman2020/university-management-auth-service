...
const getAllStudents = async (
    ...
    const result = await Student.find(whereCondition).populate(['academicSemester','academicFaculty','academicDepartment'])
      .sort(sortOption)
      .skip(skip)
      .limit(limit);
  ...
  const updateStudent = async (
    ...
    if(guardian && Object.keys(guardian).length){
      Object.keys(guardian).forEach(key=>{
        ...
        (newStudentData as any)[guardianKey] = guardian[key as keyof typeof guardian]
      })
    }
    // asign localGuardian field dynamically
    if(localGuardian && Object.keys(localGuardian).length){
      Object.keys(localGuardian).forEach(key=>{
        const localGuardianKey = `localGuardian.${key}` as keyof Partial<IStudent>; // 'name.firstName' or 'middleName' or 'name.lastName' 
        (newStudentData as any)[localGuardianKey] = localGuardian[key as keyof typeof localGuardian]
