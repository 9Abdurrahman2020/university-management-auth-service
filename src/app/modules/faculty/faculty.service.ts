...
const getAllFaculty = async (
    ...
    const result = await Faculty.find(whereCondition).populate(['academicDepartment','academicFaculty'])
      .sort(sortOption)
      .skip(skip)
      .limit(limit);
  ...
  const getSingleFaculty = async (id: string): Promise<IFaculty | null> => {
    const result = await Faculty.findOne({id}).populate(['academicDepartment','academicFaculty']);
    return result;
  };
